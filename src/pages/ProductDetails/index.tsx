        ...
        <div className='md:w-[60%] md:pl-6 md:pr-2 sm:pt-0 px-1 pt-2 w-full'>
          <div className='h-full w-full'>
            <div className='sm:px-4 pt-1 pb-1 px-1'>
              ...
              <div className='lg:hidden bg-white sticky bottom-16 flex justify-between w-full z-10 px-3 pt-3 pb-3.5 border-t border-gray-200 '>

                <div className='flex items-center justify-center w-1/2'>
                  <div className='text-2xl font-bold'>
                    <div className='relative'>
                      <span className='mx-0.5'>{product?.price ? `${Number(product.price).toFixed(2)} m.` : "-- m."}</span>
                      {Number(product?.discount) > 0 && (
                        <span className='mx-0.5 text-xs text-gray-400 line-through font-semibold translate-y-1.5'>
                          {(Number(product?.price) / (1 - Number(product?.discount) / 100)).toFixed(2)}m.
                        </span>
                      )}
                ...
                <div className='flex items-center justify-center w-1/2'>
                  <div className='py-3 px-4 flex justify-center items-center'>
                    <div>
                      <button className='bg-white px-1 mr-2 w-[42px] h-[42px] rounded-md 2border border-[#ede4ff] items-center flex justify-center' onClick={(e) => {
                        e.preventDefault();
                    ...
                    <div className='h-[42px] w-40 border border-[#0000] rounded'>
                      <button onClick={() => {
                        setopenbut4(true);
                      ...
                      }} className='h-full bg-arbat-green py-2 rounded h-9 w-full flex justify-center items-center border-y-arbat-green cursor-pointer shrink-0 hover:bg-arbat-hover text-white'>
                        {
                          openbut4 ?
                            <div className='flex items-center w-full h-full justify-between'>
                              <div>
                                <span className='px-2' onClick={() => addToCart(product)}><HiOutlineChevronLeft size={24} /></span>
                              </div>
                              <div>
                              ...
                              <div>
                                <span className='px-2' onClick={(e) => { e.stopPropagation(); azalt(); }}><HiOutlineChevronRight size={24} color='white' /></span>
                              </div>
                            </div>
                            :
